...
function addDeclToSearchResults(decl, declIndex, pkgNames, item, list, stack) {
  ...
  // add to search index
  {
      // const path = pkgNames[0] + "." + declNames.join(".");
      declSearchIndex.add(decl.name, {declIndex, declVal});
  }
  ...
  function renderSearchAPI(){
    ...
    for (let i = 0; i < term_list.length; i += 1) {
      ...
      for (let list of [result.full, result.partial]) {
        for (let r of list) {
          ...
          const match_from_start = decl_name.startsWith(term) ? -decl_name.length * (1 -ignoreCase) : (decl_name.length - term.length) + 1; 
          // being a perfect match is good
          const is_full_match = (list == result.full) ? -decl_name.length * (1 - ignoreCase) : decl_name.length - term.length;
          // matching the end of a decl name is good
          const matches_the_end = decl_name.endsWith(term) ? -decl_name.length * (1 - ignoreCase) : (decl_name.length - term.length) + 1;
    ...
    for (let idx of declSet) {
      matchedItems.push({points: declScores[idx], declIndex: idx})
    }
    ...
    for (let i = 0; i < matchedItems.length; i += 1) {
      const points = matchedItems[i].points;
      const match = matchedItems[i].declIndex;

      let canonPath = getCanonDeclPath(match);
      if (canonPath == null) continue;

      let lastPkgName = canonPath.pkgNames[canonPath.pkgNames.length - 1];
      let text = lastPkgName + "." + canonPath.declNames.join(".");
      

      const href = navLink(canonPath.pkgNames, canonPath.declNames);

      matchedItemsHTML += "<li><a href=\"" + href + "\"> (" + points + ") " +  text + "</a></li>";
    }
